...
class TestRetryDecorator:
    ...
    def test_retry_success_on_first_try(self, mock_sleep):
        """Test that with_retry returns on the first try if no error occurs."""
    ...
    @patch("time.sleep", return_value=None)
    def test_with_retry_handles_connection_error(self, mock_sleep):
        """Test that with_retry handles MeritAPIConnectionError and retries."""
        mock_func = Mock(
            side_effect=[MeritAPIConnectionError("Connection failed"), "Success"]
        )
    ...
    @patch("time.sleep", return_value=None)
    def test_with_retry_handles_rate_limit_error(self, mock_sleep):
        """Test that with_retry handles MeritAPIRateLimitError and respects retry_after."""
        mock_func = Mock(
            side_effect=[
                MeritAPIRateLimitError("Rate limit exceeded", retry_after=0.1),
                "Success",
            ]
        )
    ...
    @patch("time.sleep", return_value=None)
    def test_with_retry_fails_on_unrecoverable_error(self, mock_sleep):
        """Test that with_retry fails immediately on non-retriable errors."""
        mock_func = Mock(side_effect=ValueError("Unrecoverable error"))

        decorated_func = with_retry(
            max_retries=3, retry_on=[MeritAPIConnectionError, MeritAPIServerError]
        )(mock_func)
    ...
    def test_retry_max_retries_exceeded(self, mock_sleep):
        ...
        mock_func = MagicMock(
            side_effect=MeritAPIConnectionError("Persistent connection error")
        )

        decorated_func = with_retry(max_retries=2)(mock_func)

        with pytest.raises(MeritAPIConnectionError):
            decorated_func()
    ...
    def test_retry_backoff(self, mock_sleep):
        ...
        mock_func = MagicMock(side_effect=MeritAPIConnectionError("Connection error"))

        decorated_func = with_retry(max_retries=2, backoff_factor=1.0, jitter=False)(
            mock_func
        )
